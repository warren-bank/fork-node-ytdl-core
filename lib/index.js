...
const { parseTimestamp, convertTimestampToByteOffset } = require('./time-utils');
...
const downloadFromInfoCallback = (stream, info, options) => {
  ...
  if (format.isHLS || format.isDashMPD) {
    ...
    if (!(options.range && (options.range.start || options.range.end)) && options.timerange && (options.timerange.start || options.timerange.end)) {
      options.range = {
        start: convertTimestampToByteOffset(format, options.timerange.start),
        end:   convertTimestampToByteOffset(format, options.timerange.end)
      }
    }
